	...
	private function  insertSingle($model,$filter){
		...
		if($this->$model->insert($this->db,$message)){
			...
			if($this->DoAfterInsertion($model,'insert',$data,$this->db,$message)){
				...
				// this is to suspend the normal result,useful for redirecting to another page
				// if($this->suspendAfterInsertionResult($model,'suspend',$data,$this->db,$message)){
				// 	return;
				// }
...
// this is the calendar controller method
public function calendarLoad()
 {
     ...
     $events = $this->modelCalendar->get_events($start_format, $end_format);
...
public function add_event() 
{
    ...
    $this->modelCalendar->add_event(array(
       "title" => $name,
       "description" => $desc,
       "start" => $start_date,
       "end" => $end_date,
